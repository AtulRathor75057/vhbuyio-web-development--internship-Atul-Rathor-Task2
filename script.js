...
function getMarqueeBounds() {
    ...
    return {
        x,
        y,
        width: Math.abs(state.marqueeCurrentX - state.marqueeStartX),
        height: Math.abs(state.marqueeCurrentY - state.marqueeStartY)
    };
}

function getRotatedObjectAABB(obj) {
    ...
    ].map(pt => rotatePoint(pt.x, pt.y, center.x, center.y, obj.rotation || 0));
...
function selectObjectsInMarquee() {
    const marquee = getMarqueeBounds();
    if (marquee.width < 3 && marquee.height < 3) return;

    const selected = state.marqueeAdditive
        ? new Set(state.selectedIds)
        : new Set();

    state.objects.forEach(obj => {
        if (obj.visible === false || obj.locked === true || obj.type === "group") return;
        const bounds = getRotatedObjectAABB(obj);
        if (rectanglesIntersect(marquee, bounds)) selected.add(obj.id);
    });
...
function drawMarqueeSelection() {
    ...
    const b = getMarqueeBounds();
    const topLeft = canvasToScreen(b.x, b.y);
    const width = b.width * state.zoom;
    const height = b.height * state.zoom;
...
// Public tool API used by keyboard shortcuts and tests.
function setActiveTool(tool) {
    const allowed = ["select", "rectangle", "ellipse", "line", "text"];
    ...
    function(e) {
        ...
        if (
            ...
            if (hit) {
                ...
                state.marqueeStartX = point.x;
                state.marqueeStartY = point.y;
                state.marqueeCurrentX = point.x;
                state.marqueeCurrentY = point.y;
                state.marqueeAdditive = Boolean(e.shiftKey);
...
function groupSelected() {
    const selected = getSelectedObjects().filter(obj => obj.type !== "group");
    if (selected.length < 2) return;

    saveHistory();

    const bounds = selected.reduce((acc, obj) => {
        const b = getRotatedObjectAABB(obj);
    ...
    const group = {
        ...
        locked: false,
        parentId: null,
        children: selected.map(obj => obj.id)
    };
    ...
    // Keep the group above its children so it is easy to find in Layers.
...
function saveHistory() {
    ...
    // saveHistory() is used as the "before" checkpoint. Avoid duplicate
    // snapshots when a tool is pressed repeatedly or a wrapper also commits.
...
// commitHistory() is the public "after" checkpoint used by all action hooks.
...
function loadProjectJSON(file) {
    ...
    reader.onload = function (event) {
        ...
        try {
            ...
            clearHistory();
            historyState.undoStack.push(createHistorySnapshot());
            updateHistoryButtons();
...
const phase6OriginalGroup = groupSelected;
...
const phase6OriginalUngroup = ungroupSelected;
...
const phase6PropertiesContent = document.getElementById("propertiesContent");

if (phase6PropertiesContent) {

    phase6PropertiesContent.addEventListener("focusin", function () {
        if (!historyState.isRestoring) saveHistory();
    });

    phase6PropertiesContent.addEventListener(
        "change",
        function () {
